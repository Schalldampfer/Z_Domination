...
_poss set [2, 0];
private _vec = createVehicle [d_main_tower, _poss, [], 0, "NONE"];
...
["zombie", [_poss], _trg_center, 0, "guard", d_enemy_side_short, 0, -1.111, 1, [_trg_center, _mtradius]] call d_fnc_makegroup;
...
//barrack test
d_f_check_trigger = [_vec, [50, 50, 0, false, 10], ["ANYPLAYER", "PRESENT", true], ["this", "", ""]] call d_fnc_createtriggerlocal;
_vec setVariable ["d_bar_trig", d_f_check_trigger];
d_bara_trig_ar pushBack d_f_check_trigger;
...
if (d_ao_check_for_ai in [0, 1]) then {
	...
	private _parray = [_trg_center, d_cur_target_radius, 4, 0.3, 0, false, true] call d_fnc_GetRanPointCircleBigArray;
	...
	for "_i" from 1 to _nrcamps do {
		...
		if (_wf distance2D _trg_center > d_cur_target_radius || {random 100 > 30}) then {
			private _retgr = ["zombie", [_poss], _trg_center, 0, "guard", d_enemy_side_short, 0, -1.111, 1, [_trg_center, _mtradius]] call d_fnc_makegroup;
			(_retgr # 1) spawn {
