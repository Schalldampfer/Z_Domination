...
private _garrisonUnits = {
	...
	private _unitlist = ["zombie", d_enemy_side_short] call d_fnc_getunitlistm;
	...
	*/
	...
	if (d_mt_respawngroups == 0) then {
		...
		_newgroup setVariable ["d_respawninfo", ["zombie", [], _trg_center, 0, "patrol2", d_side_enemy, 0, 0, 1, [_trg_center, _radius], false, []]];
	};//
...
private _type_list_guard = [
	["zombie", 0, [d_footunits_guard, round(random 1)] call _selectitmen]
];

private _type_list_guard_static = [
	["zombie", 0, [d_footunits_guard_static, round(random 1)] call _selectitmen]
];

private _type_list_patrol = [
	["zombie", 0, [d_footunits_patrol, 0] call _selectitmen],
	["zombie", 0, [d_footunits_patrol, 1] call _selectitmen]
];

private _type_list_guard_static2 = [
	["zombie", 0, [d_footunits_attack, 0] call _selectitmen],
	["zombie", 0, [d_footunits_attack, 1] call _selectitmen]
];

__TRACE_1("","_type_list_guard")

params ["_trgobj", "_radius"];
private _patrol_radius = _radius + random 300;
...
d_num_barracks_objs = ((ceil random d_max_bar_cnt) max (ceil d_max_bar_cnt/2)) min d_max_bar_cnt;
__TRACE_1("","d_num_barracks_objs")
d_mt_barracks_obj_ar = [];

private _parray = [_trg_center, d_cur_target_radius, 11, 0.7, 0, false, true] call d_fnc_GetRanPointCircleBigArray;
...
private _comppost = [];
{
	if ((_x # 0) call _fnc_dospawnr) then {
		...
		for "_xxx" from 1 to (_x # 2) do {
			...
			if (!isNil "d_compositions" && {!(d_compositions isEqualTo []) && {(_x # 0) in ["zombie", "demon"]}}) then {
...

{
	if ((_x # 0) call _fnc_dospawnr) then {
		...
		for "_xxx" from 1 to (_x # 2) do {
			...
			private _ppos = _curar select _wp_ran;
			private _iscompost = false;
			if (!isNil "d_compositions" && {(_x # 0) in ["zombie", "demon"]}) then {
...

{
	if ((_x # 0) call _fnc_dospawnr) then {
		...
		for "_xxx" from 1 to (_x # 2) do {
			private _wp_ran = (count _curar) call d_fnc_RandomFloor;
			[_x # 0, [_curar select _wp_ran], _trg_center, _x # 1, ["patrol", "patrol2mt"] select (_x # 0 == "zombie" || {_x # 0 == "demon"}), d_enemy_side_short, 0, -1.111, 1, [_trg_center, _patrol_radius]] call d_fnc_makegroup;
			_curar deleteAt _wp_ran;
			sleep 0.2;
		};
	};
