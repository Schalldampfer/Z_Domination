...
if (!d_gmcwg) then {
	_items_no append ["FirstAidKit", "Medikit","ItemCompass", "ItemWatch", "ToolKit", "B_UavTerminal"];
};

if (!d_ifa3lite && {!d_gmcwg && {!d_unsung}}) then {
	_items_no append ["Rangefinder", "NVGoggles", "NVGoggles_OPFOR", "NVGoggles_INDEP", "ItemGPS", "MineDetector", "Laserdesignator"];
};
...
if (d_with_ace) then {
	_items_no append ["ACE_atropine", "ACE_fieldDressing", "ACE_elasticBandage", "ACE_quikclot", "ACE_bloodIV", "ACE_bloodIV_500", "ACE_bloodIV_250",
		...
		"ACE_RangeCard", "ACE_MapTools", "ACE_DefusalKit", "ACE_Clacker"];
};

#ifdef __ZOMBIE__
	_items_no append ["RyanZombiesAntiVirusCure_Item", "RyanZombiesAntiVirusTemporary_Item"];
#endif
